...
- **Commit bookkeeping:** your own writes — `docs/sprints/`, `docs/handoff-queue.md`, `docs/plans/` — go straight to `<plan-slug>` and get pushed, never through a PR (they're records, not code; gating them buys nothing). Commit at the end of each wave, before integrating, so the wave head carries the board. Never stage anything outside `docs/`; an unrelated dirty file → leave it and note it.
...
**Resume a halted wave:** re-dispatch each `blocked` slice fresh — reset its worktree if it exists (per convention) else recreate it (step 2); dispatch (step 3) with any still-`pending` slice. Skip `pushed`, `merged`, and `done` — a `pushed` slice's branch is already on origin, and re-dispatching it would fail step 2's `git worktree add -b` on the existing branch, turning a success into a `blocked`.
...
6. **Commit bookkeeping** per convention — steps 4 and 5 wrote to the parent repo's working tree, and nothing else commits them.
...
- **Dispatch:** pre-create `.claude/worktrees/<sprint-slug>-review/` off `origin/<plan-slug>`, then dispatch per convention with the sprint-slug and the sprint's merged slice branches.
...
- **Update the plan & advance:** set the sprint's row in `docs/plans/<plan-slug>.md` to `done` (reshape later rows only if unresolved `PENDING`s require it — never rewrite history), then **commit bookkeeping** per convention — the archive `mv`, the summary, the reviewer's triaged concerns, and this row are all uncommitted until you do, and the final plan PR is what carries them to `<merge-target>`. If a `planned` sprint row remains → end the turn: `Sprint <sprint-slug> complete. Reply 'continue' to start the next sprint.` If none remains → go to **Plan complete**.
